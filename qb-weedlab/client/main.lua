...
function SetClosestWeedlab()
    local pos = GetEntityCoords(PlayerPedId(), true)
    local current = nil
    local dist = nil
    for id, weedlab in pairs(Config.Locations["laboratories"]) do
        if current ~= nil then
        ...
        if InsideWeedlab then
            if(Vdist(pos, Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z, true) < 20)then
                ...
                if(Vdist(pos, Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z, true) < 1)then
                    DrawText3Ds(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z, '~g~E~w~ - Leave Weedlab')
                            ...
                            if(Vdist(pos, Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, true) < 20)then
                                ...
                                if(Vdist(pos, Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, true) < 1)then
                                    if CarryPackage == nil then
                                        DrawText3Ds(Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, '~g~E~w~ - Grab Boxes')
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
                                            QBCore.Functions.Progressbar("pickup_reycle_package", "Grabbing boxes..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                                        ...
                                        DrawText3Ds(Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, '~g~E~w~ - Put away boxes')
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            DropIngredients()
                                        end
                                    end
                ...
                if CurrentTask ~= 0 then
                    if(Vdist(pos, Config.Tasks[CurrentTask].coords.x, Config.Tasks[CurrentTask].coords.y, Config.Tasks[CurrentTask].coords.z, true) < 20)then
                        ...
                        if(Vdist(pos, Config.Tasks[CurrentTask].coords.x, Config.Tasks[CurrentTask].coords.y, Config.Tasks[CurrentTask].coords.z, true) < 1)then
                            ...
                            DrawText3Ds(Config.Tasks[CurrentTask].coords.x, Config.Tasks[CurrentTask].coords.y, Config.Tasks[CurrentTask].coords.z - 0.2, Config.Tasks[CurrentTask].ingredients.current..'/'..Config.Tasks[CurrentTask].ingredients.needed..' Boxes')
                            if Config.Tasks[CurrentTask].ingredients.current == Config.Tasks[CurrentTask].ingredients.needed then
                                if not Config.Tasks[CurrentTask].started then
                                    if not Config.Tasks[CurrentTask].done then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            ...
                                            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
                                            QBCore.Functions.Progressbar("fullcut_cycle_v6_weedcutter", "Processing..", 12000, false, true, {}, {}, {}, {}, function() -- Done
                                                ClearPedTasks(PlayerPedId())
                                            end)
                                        end
                                    else -- alle ingredienten zitten erin
                                        if CurrentTask == 2 then
                                        DrawText3Ds(Config.Tasks[CurrentTask].coords.x, Config.Tasks[CurrentTask].coords.y, Config.Tasks[CurrentTask].coords.z - 0.4, '[E] Grab Weed Bricks')
                                            if IsControlJustPressed(0, Keys["E"]) then
                                                TriggerServerEvent('qb-weedlab:server:getItem')
                                ...
                                if CurrentTask < 3 then
                                    if CarryPackage ~= nil then
                                        if Config.Tasks[CurrentTask].ingredients.current < Config.Tasks[CurrentTask].ingredients.needed then
                                            DrawText3Ds(Config.Tasks[CurrentTask].coords.x, Config.Tasks[CurrentTask].coords.y, Config.Tasks[CurrentTask].coords.z + 0.2, '[E] Place Boxes')
                                            if IsControlJustPressed(0, Keys["E"]) then
                                                DropIngredients()
...
RegisterNetEvent('qb-weedlab:client:UseLabKey', function(weedkey)
    if ClosestWeedlab == Config.CurrentLab then
        ...
        local dist = Vdist(pos, Config.Locations["laboratories"][ClosestWeedlab].coords.x, Config.Locations["laboratories"][ClosestWeedlab].coords.y, Config.Locations["laboratories"][ClosestWeedlab].coords.z, true)
        if dist < 1 then
            if weedkey == ClosestWeedlab then
                EnterWeedlab()
            else
